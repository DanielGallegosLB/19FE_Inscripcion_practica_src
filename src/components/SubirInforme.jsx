...
function SubirInforme() {
  ...
  const handleFileUpload = async () => {
    ...
    guardarInformeEnAPI(auth.user, auth.id, url, new Date().toISOString());
  ...
  const guardarInformeEnAPI = async (usuario, id, linkDescarga) => {
    try {
            ...
            {files.map((file, index) => {
              ...
              return (
                <tr key={index}>
                  ...
                  <td>
                    <button onClick={() => handleFileDelete(fileRef)}>Eliminar</button>
                  </td>
                </tr>
              );
            })}
